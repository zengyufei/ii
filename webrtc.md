...
在 `Send to` 中选择已连接的设备，在 `Text message` 多行输入框输入内容后发送。文本只发送给当前选中的设备；接收端会在当前页面的 `Received messages` 列表中显示来源设备和完整内容，每条消息都有 `Copy` 按钮。列表只存在于当前页面，刷新后清空，`ii` 不保存消息或建立聊天记录。

文本先编码为 UTF-8 字节，再按不超过 1 MiB 的分片发送；接收端校验长度后重新组装成一条消息，因此中文、换行和长文本都不会按分片显示。文件发送仍使用原有文件控件和协议。
