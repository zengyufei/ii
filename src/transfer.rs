use crate::{
    cli::{RecvArgs, SendArgs, TunnelArgs, WebArgs, WebrtcArgs},
    storage,
    ticket::{
        FtpPortableCredentials, PayloadKind, ResumeRequest, SftpPortableAuth,
        SftpPortableCredentials, Ticket, TunnelRelayMode, WebDavPortableCredentials,
    },
};
...
const TUNNEL_ALPN: &[u8] = b"ii/tunnel/1";
...
async fn bind_tunnel_listener_from(start_port: u16) -> Result<TcpListener> {
    ...
    F: Fn(&str) -> Result<()> + Send + Sync,
{
    ...
    let source = Source::open(args.path.clone(), args.name.clone()).await?;
    let endpoint = bind_endpoint(endpoint_policy_for_send(&args)?, ALPN).await?;
...
pub async fn recv(args: RecvArgs) -> Result<()> {
    ...
    let endpoint = bind_endpoint(policy, ALPN).await?;
...
async fn bind_endpoint(policy: EndpointPolicy, alpn: &[u8]) -> Result<Endpoint> {
