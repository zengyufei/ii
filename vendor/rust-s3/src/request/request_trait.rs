...
fn md5_digest(bytes: impl AsRef<[u8]>) -> [u8; 16] {
    let mut ctx = <md5::Md5 as md5::Digest>::new();
    md5::Digest::update(&mut ctx, bytes.as_ref());
...
pub trait Request {
    ...
    async fn headers(&self) -> Result<HeaderMap, S3Error> {
        ...
        // headers.insert(header::ACCEPT_CHARSET, HeaderValue::from_str("UTF-8")?);
            ...
            let digest = md5_digest(to_string(configuration)?.as_bytes());
            let hash = general_purpose::STANDARD.encode(digest);
            headers.insert(HeaderName::from_static("content-md5"), hash.parse()?);
        ...
        {
            let digest = md5_digest(configuration.to_string().as_bytes());
            let hash = general_purpose::STANDARD.encode(digest);
            headers.insert(HeaderName::from_static("content-md5"), hash.parse()?);
        ...
        } = self.command()
        {
            ...
            let body = data.to_string();
            let digest = md5_digest(body.as_bytes());
            let hash = general_purpose::STANDARD.encode(digest);
            headers.insert(HeaderName::from_static("content-md5"), hash.parse()?);
        }
