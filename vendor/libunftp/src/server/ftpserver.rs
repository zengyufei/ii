...
use slog::*;
use std::{ffi::OsString, fmt::Debug, future::Future, net::SocketAddr, ops::RangeInclusive, pin::Pin, sync::Arc, time::Duration};
    ...
    User: UserDetail + 'static,
{
    ...
    /// Limits concurrent FTP control connections.
    ...
    /// Enables explicit FTPS with an already-built TLS configuration.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    /// Enables implicit FTPS with an already-built TLS configuration.
    pub fn ftps_implicit_manual(mut self, config: Arc<ServerConfig>) -> Self {
        self.ftps_mode = FtpsConfig::Implicit { tls_config: config };
        self
    }
