    ...
    Storage::Metadata: Metadata,
{
    ...
    let implicit_tls = matches!(ftps_config, FtpsConfig::Implicit { .. });
    let tls_configured = matches!(ftps_config, FtpsConfig::On { .. } | FtpsConfig::Implicit { .. });
    ...
    let control_stream: Box<dyn AsyncReadAsyncWriteSendUnpin> = match ftps_config.clone() {
        FtpsConfig::Implicit { tls_config } => {
            let stream = TlsAcceptor::from(tls_config)
                .accept(tcp_stream)
                .await
                .map_err(|err| std::io::Error::other(format!("implicit FTPS handshake failed: {err}")))?;
            Box::new(stream)
        }
        _ => Box::new(tcp_stream),
    };
    let mut session: Session<Storage, User> = Session::new(Arc::new(storage), peer_addr)
    ...
    let cmd_and_reply_stream: Framed<Box<dyn AsyncReadAsyncWriteSendUnpin>, FtpCodec> = codec.framed(control_stream);
