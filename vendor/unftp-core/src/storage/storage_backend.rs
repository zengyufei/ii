...
impl std::fmt::Display for Permissions {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_char(if self.0 & PERM_USER & PERM_READ > 0 {
            'r'
        } else {
            '-'
        })?;
        f.write_char(if self.0 & PERM_USER & PERM_WRITE > 0 {
            'w'
        } else {
            '-'
        })?;
        f.write_char(if self.0 & PERM_USER & PERM_EXEC > 0 {
            'x'
        } else {
            '-'
        })?;
        f.write_char(if self.0 & PERM_GROUP & PERM_READ > 0 {
            'r'
        } else {
            '-'
        })?;
        f.write_char(if self.0 & PERM_GROUP & PERM_WRITE > 0 {
            'w'
        } else {
            '-'
        })?;
        f.write_char(if self.0 & PERM_GROUP & PERM_EXEC > 0 {
            'x'
        } else {
            '-'
        })?;
        f.write_char(if self.0 & PERM_OTHERS & PERM_READ > 0 {
            'r'
        } else {
            '-'
        })?;
        f.write_char(if self.0 & PERM_OTHERS & PERM_WRITE > 0 {
            'w'
        } else {
            '-'
        })?;
        f.write_char(if self.0 & PERM_OTHERS & PERM_EXEC > 0 {
            'x'
        } else {
            '-'
        })?;
...
pub trait StorageBackend<User: UserDetail>: Send + Sync + Debug {
    ...
    async fn list_fmt<P>(
        ...
        user: &User,
        path: P,
    ) -> std::result::Result<std::io::Cursor<Vec<u8>>, Error>
    where
        P: AsRef<Path> + Send + Debug,
    ...
    async fn nlst<P>(
        ...
        user: &User,
        path: P,
    ) -> std::result::Result<std::io::Cursor<Vec<u8>>, std::io::Error>
    where
        P: AsRef<Path> + Send + Debug,
        Self::Metadata: Metadata + 'static,
    {
        ...
        P: AsRef<Path> + Send + Debug,
    {
        ...
        Ok(tokio::io::copy(&mut reader, output)
            .await
            .map_err(Error::from)?)
    }
...
impl From<std::io::Error> for Error {
    fn from(err: std::io::Error) -> Self {
        ...
        match (kind, raw_os_error) {
            ...
            (std::io::ErrorKind::ConnectionReset, _) => {
                Error::new(ErrorKind::ConnectionClosed, err)
            }
            ...
            (std::io::ErrorKind::ConnectionAborted, _) => {
                Error::new(ErrorKind::ConnectionClosed, err)
            }
