...
mod auth {
    ...
    use unftp_core::auth::{
        AuthenticationError, Authenticator, Principal, UserDetail, UserDetailError,
        UserDetailProvider,
    };
    ...
    impl JsonFileAuthenticator {
        ...
        pub fn from_json<T: Into<String>>(json: T) -> Result<Self, Box<dyn std::error::Error>> {
            let credentials_list: Vec<Credentials> =
                serde_json::from_str::<Vec<Credentials>>(&json.into())?;
            let map: Result<HashMap<String, UserCreds>, _> = credentials_list
                .into_iter()
                .map(Self::list_entry_to_map_entry)
                .collect();
        ...
        fn list_entry_to_map_entry(
            user_info: Credentials,
        ) -> Result<(String, UserCreds), Box<dyn std::error::Error>> {
            let map_entry = match user_info {
                Credentials::Plaintext {
                    username,
                    password,
                    home,
                } => (username.clone(), UserCreds { password, home }),
            };
            Ok(map_entry)
        }
...
async fn main() {
    ...
    let mut sb = libunftp::ServerBuilder::with_authenticator(sgen, authenticator)
        .user_detail_provider(user_provider);
    cfg_if! {
        if #[cfg(target_os = "freebsd")] {
