...
async fn put(#[future] harness: Harness) {
    ...
    let remote_data = ftp_stream
        .simple_retr("greeting.txt")
        .await
        .unwrap()
        .into_inner();
...
mod list {
    ...
    async fn format(#[future] harness: Harness) {
        ...
        for entry in list {
            if entry.contains("test.txt") {
                assert!(
                    re.is_match(&entry),
                    "\"{entry}\" did not match pattern {re:?}"
                );
                return;
            }
        }
    ...
    async fn symlink(
        ...
        let list = ftp_stream.list(None).await.unwrap();
        let pat =
            format!("^l[rwx-]{{9}}\\s+{link_count}\\s+{uid}\\s+{gid}\\s+{size}.*link -> {target}");
        let re = Regex::new(&pat).unwrap();
        for entry in list {
...
mod mdtm {
    ...
    async fn regular(#[future] harness: Harness) {
        ...
        assert_eq!(
            r.to_rfc2822(),
            chrono::DateTime::<chrono::Utc>::from(modified).to_rfc2822()
        );
    }
...
async fn dele(#[future] harness: Harness) {
    ...
    assert_eq!(
        std::fs::metadata(file_in_root.path()).unwrap_err().kind(),
...
async fn rmd(#[future] harness: Harness) {
    ...
    assert_eq!(
        std::fs::metadata(dir_in_root.path()).unwrap_err().kind(),
        std::io::ErrorKind::NotFound
    );
}
...
async fn rename(#[future] harness: Harness) {
    ...
    let mut ftp_stream = FtpStream::connect(harness.addr)
        .await
...
mod mlsd {
    ...
    async fn connect_pasv(ftp: &mut FtpStream) -> TcpStream {
        ...
        TcpStream::connect(format!("127.0.0.1:{}", data_port))
            .await
            .unwrap()
    }
    ...
    fn check_facts(line: &str, expected: &[&str]) {
        ...
        assert_eq!(
            parts.len(),
            2,
            "Line should have facts and filename separated by space"
        );
    ...
    async fn basic_mlsd(#[future] harness: Harness) {
        ...
        assert!(
            entries.pop().unwrap().is_empty(),
            "Last line should be empty due to trailing CRLF"
        );
        ...
        let dir_line = *entries
            .iter()
            .find(|line| line.contains(" test_dir"))
            .expect("Should return test_dir");
